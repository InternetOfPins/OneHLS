...
def _bambu_cmd():
    ...
    outdir = os.path.join(HERE, ".hls_out_cic_decimator")
    os.makedirs(outdir, exist_ok=True)
    src = os.path.join(HERE, "hls", "cic_decimator_top.cpp")
    return (
        f'cd "{outdir}" && "{BAMBU}" '
        ...
        f'--top-fname=cicDecimatorTop -v2 "{src}"'
    )


env.AddCustomTarget(
    name="synthesize-cic-decimator",
    dependencies=None,
    actions=[_bambu_cmd()],
    title="HLS: synthesize a CIC decimator (N=2, R=4)",
    description="CicDecimator<ac_int<32,true>,ac_int<32,true>,2,4> -- see "
                 "../../docs/PHASE4_CIC_EXPERIMENT.md for the full "
                 "derivation and the composed-vs-hand-written-monolithic "
                 "resource comparison this example's README numbers come from.",
    always_build=True,
)
