proc dump_statistics {  } {
  ...
  regexp --  {\s*RAMS32\s*\|\s*([^[:blank:]]+)} $util_rpt ignore RAMS32
  regexp --  {\s*RAMD32\s*\|\s*([^[:blank:]]+)} $util_rpt ignore RAMD32
  regexp --  {\s*RAMD64E\s*\|\s*([^[:blank:]]+)} $util_rpt ignore RAMD64E
  set DRAM [expr {$RAMS32 + $RAMD32 + $RAMD64E}]
  ...
  file delete -force HLS_output/Synthesis/vivado_flow/oneHlsFirTop_report.xml 
  set ofile_report [open HLS_output/Synthesis/vivado_flow/oneHlsFirTop_report.xml w]
...
create_project oneHlsFirTop -part xc7a100tcsg324-1 -force
read_verilog {oneHlsFirTop.v} {oneHlsFirTop_mac_muladd_16s_7ns_20s_23_4_1.v} {oneHlsFirTop_mac_muladd_16s_7ns_24s_24_4_1.v}
set rfile_sdc [open HLS_output/Synthesis/vivado_flow/oneHlsFirTop.sdc r]
set clk_constraint [regexp -all --  CLK_SRC [read $rfile_sdc [file size HLS_output/Synthesis/vivado_flow/oneHlsFirTop.sdc]]]
if { [expr {$clk_constraint == 0}] } {
  set ofile_sdc [open HLS_output/Synthesis/vivado_flow/oneHlsFirTop.sdc a]
  puts $ofile_sdc "set_property HD.CLK_SRC BUFGCTRL_X0Y0 \[get_ports clock\]"
  close $ofile_sdc
}
read_xdc HLS_output/Synthesis/vivado_flow/oneHlsFirTop.sdc
synth_design -mode out_of_context -no_iobuf -top oneHlsFirTop -part xc7a100tcsg324-1
